...
class AgentNodes:
    ...
    def analyze_intent(self, state: QueryState) -> QueryState:
        ...
        try:
            ...
            logger.info(
                f"[intent] analyzed question; type={intent.intent_type.value}, time_scope={intent.time_scope.value}, aggs={len(intent.aggregations)}, filters={len(intent.filters)}"
            )
            entities = [
                {
                    "text": e.text,
    ...
    def map_schema(self, state: QueryState) -> QueryState:
        ...
        try:
            ...
            state.tables = tables
            logger.info(f"[schema] mapped entities to {len(tables)} table(s): {tables}")
            return state
        except Exception as e:
    ...
    def plan_query(self, state: QueryState) -> QueryState:
        ...
        try:
            ...
            logger.info(f"[planner] produced plan for {len(state.tables)} table(s)")
            return state
        except Exception as e:
