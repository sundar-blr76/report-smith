...
from reportsmith.query_processing.sql_generation.models import (
    AggregationType,
    IntentType,
    SQLColumn,
    SQLJoin,
...
from reportsmith.query_processing.sql_generation.utils import (
    normalize_filter_value,
    normalize_column_reference,
    detect_llm_provider,
    detect_llm_model,
)
...
class SQLGenerator:
    ...
    def __init__(
        ...
        if llm_client:
            self.llm_provider = detect_llm_provider(llm_client)
            self.llm_model = detect_llm_model(llm_client)
            ...
            if len(equality_filters) > 1:
                ...
                for operator, value, filter_str in col_filters:
                    ...
                    for op, val, fs in col_filters:
                        ...
                        if match:
                            normalized_col = normalize_column_reference(
                                match.group(1), entities, self.kg
                            )
                            break
                    
                    if not normalized_col:
                        # Fallback to column name
                        normalized_col = normalize_column_reference(col_name, entities, self.kg)
                    
                    # Normalize value (handle 100M, 1K, etc.)
                    normalized_value = normalize_filter_value(f"'{value}'")
...
"""
            ...
            provider_type = detect_llm_provider(self.llm_client)
