#!/bin/bash
# Run the query processing example with proper environment setup and logging
...
LOG_FILE="$LOGS_DIR/query_example_${TIMESTAMP}.log"

echo -e "${GREEN}Running ReportSmith Query Processing Example${NC}"
...
# Run the example with tee to show output and save to log
echo -e "${GREEN}Starting query processing example...${NC}"
echo ""
...
python3 "$SCRIPT_DIR/query_processing_example.py" 2>&1 | tee -a "$LOG_FILE"
